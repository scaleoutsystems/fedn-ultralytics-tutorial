...
def build_client_folder(folder, data, idx, subdir):
    ...
    for image in data[x][idx]:
        old_image_path = os.path.join(f"{abs_path}/../../datasets/african-wildlife/{folder}/images", image)
        new_image_path = os.path.join(f"{subdir}/{folder}/images", image)
        shutil.move(old_image_path, new_image_path)
    for label in data[y][idx]:
        old_image_path = os.path.join(f"{abs_path}/../../datasets/african-wildlife/{folder}/labels", label)
...
def split(out_dir="data"):
    ...
    X_train = [f for f in os.listdir(f"{abs_path}/../../datasets/african-wildlife/train/images")]
    X_test = [f for f in os.listdir(f"{abs_path}/../../datasets/african-wildlife/test/images")]
    X_val = [f for f in os.listdir(f"{abs_path}/../../datasets/african-wildlife/valid/images")]

    y_train = [f for f in os.listdir(f"{abs_path}/../../datasets/african-wildlife/train/labels")]
    y_test = [f for f in os.listdir(f"{abs_path}/../../datasets/african-wildlife/test/labels")]
    y_val = [f for f in os.listdir(f"{abs_path}/../../datasets/african-wildlife/valid/labels")]
    ...
    for i in range(n_splits):
        ...
        if not os.path.exists(subdir):
            ...
            move_data_yaml(f"{abs_path}", subdir)
    # Remove downloaded directory
    #if os.path.exists(f"{abs_path}/datasets"):
    #    shutil.rmtree(f"{abs_path}/datasets")
